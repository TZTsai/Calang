...
* Evaluation of boolean expressions  
    ...
    **Examples**: 3 > 2, 2 = 2, x = y xor x = z, x > 0 and x < 2, not (a or b) = (not a and not b), 1 /\ 0, f \/ g
    (note: /\ and \/ resemble the symbols for "logical and" and "logical or" in mathematics. They are the same as `and` and `or` operations except higher priority.)
...
* Conditional expression  
    ...
    **Examples**:  
     ...
     **Syntax 2**: cases(\<cond1>: \<exp1>, \<cond2>: \<exp2>, ... (, \<expElse>))

    (the optional \<expElse> does not have a condition preceding it)  
